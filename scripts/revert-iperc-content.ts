/**
 * Script para revertir contentJson de IPERC a versión simplificada
 *
 * Uso: npx tsx scripts/revert-iperc-content.ts
 */
...
async function main() {
  ...
  console.log(`Revirtiendo: ${lesson.title} (${lesson.id})`)

  await prisma.lesson.update({
    where: { id: lesson.id },
    data: {
      contentJson: originalContentJson as Parameters<typeof prisma.lesson.update>[0]['data']['contentJson']
    }
  })

  console.log('✅ Revertido a versión simplificada')
}
